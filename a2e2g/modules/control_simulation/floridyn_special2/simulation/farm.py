...
class Farm:
    ...
    def set_axial_induction(self, axial_induction):
        """
        Sets the yaw angles for all turbines on the
        :py:obj:`~.turbine.Turbine` objects directly.
        ...
        Args:
            yaw_angles (float or list( float )): A single value to set
                ...
                in degrees.
        """
        if isinstance(axial_induction, float) or isinstance(axial_induction, int):
            axial_induction = [axial_induction] * len(self.turbines)

        for ai, turbine in zip(axial_induction, self.turbines):
            turbine.ai_set = ai    
