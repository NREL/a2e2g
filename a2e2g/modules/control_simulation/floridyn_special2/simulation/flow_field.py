...
class FlowField:
    ...
    def _compute_initialized_domain(self, with_resolution=None, points=None):
        ...
        if with_resolution is not None:
            ...
            if points is not None:
                ...
                # wind field buffer is only used for visualization purposes, only needs to be created if points is not None (meaning, I think, that fi.get_hor_plane() was called, among other scenarios)
    ...
    def _find_turb_num(self, turb_num):
        return self.turbine_map.coords[turb_num] # MS CHANGED 1/11/21
        # for turbine in self.turbine_map.turbines:
        #     if turbine.number == turb_num:
        #         return self.turbine_map.coords[turb_num]
        
        raise ValueError("Invalid turbine number")

    def propagate_wake_effects(self, turb_u_wake, index, sim_time, first_x=None, angle_changes=None):
        """
        Sends wake effects to the buffers of downstream turbines.

        Args:
            turb_u_wake: The wake effect to propagate.
            index: The index of the turbine that is propagating the wake effects.
            sim_time: The current simulation time.
            first_x: x coordinate of the first turbine. If None, this will be assumed to be the smallest x coordinate in the wind field. This can be used to have propagation originate from different points in the wind field, such as individual turbines.
            angle_changes: List of booleans specifying which turbines have changed their yaw angles.
        """
        ...
        # set comparison_x variable using first_x and angle_changes in order to calculate distance d for Taylor's frozen wake hypothesis
        comparison_x = first_x
        for (temp_coord, temp_turbine) in sorted_map:
            if temp_turbine.number == index:
                comparison_x = temp_coord.x1
        
        if angle_changes is not None and any(angle_changes): # MS changed
            turb_xs = []
            for i,angle_change in enumerate(angle_changes):
                if angle_change:
                    coord = self._find_turb_num(i)

                    turb_xs.append(coord.x1)
        ...
        for i, (temp_coord,temp_turbine) in enumerate(sorted_map):
            if temp_coord.x1 > comparison_x and index != temp_turbine.number:
                ...
                U_inf = self.mean_wind_speed#wind_speed
                tau = x / U_inf
    ...
    def propagate_wind_directions(self, old_turbine_wind_direction, new_wind_direction, sim_time, first_x=None):
        """
        Sends wind directions to the buffers of downstream turbines.

        Args:
            old_turbine_wind_direction: The current wind direction, which may be overwritten by the new wind direction.
            new_wind_direction: Wind direction that will be added to the buffer.
    ...
    def propagate_wind_speeds(self, old_input_wind_speed, new_wind_speed, sim_time, first_x=None):
        """
        Sends wind speeds to the buffers of downstream turbines.

        Args:
            old_input_wind_speed: List of wind speeds corresponding to current state of the wind field, which may be overwritten by the new wind direction.
            new_wind_speed: Wind speed that will be added to the buffer.
            sim_time: Current simulation time.
            first_x: x coordinate of the first turbine. If None, this will be assumed to be the smallest x coordinate in the wind field. This can be used to have propagation originate from different points in the wind field, such as individual turbines.
        """
        # define the center of rotation with reference to 270 deg
        ...
        for i, (temp_coord,temp_turbine) in enumerate(sorted_map):
            ...
            if len(old_input_wind_speed) == 1:
                ...
                old_wind_speed = old_input_wind_speed[i]
            ...
            temp_turbine.wind_field_buffer.initialize_wind_speed(old_wind_speed, new_wind_speed, overwrite=overwrite)

            if temp_coord.x1 > first_x:
                x = temp_coord.x1 - first_x
                U_inf = self.mean_wind_speed
                tau = x / U_inf
                delay = round(tau) + sim_time
    ...
    def calculate_wake(self, no_wake=False, points=None, track_n_upstream_wakes=False, sim_time=None, look_ahead=False, propagate_wind_speed=None, visualize=False, first_x=None, angle_changes=None):
        ...
        Args:
            ...
            look_ahead (bool): Specifies whether steady-state 
                solution should be calculated and bufferred 
                into the wind farm.
            propagate_wind_speed (double): The wind speed that
                wake effects should be propagated at. This allows
                for the case in which wake effects are interpreted
                as eddies within a larger, mean wind speed flow.
            visualize (bool): Specifies whether a "visualization" 
                buffer should be used, which tracks individual 
                wind speeds at every point in the wind field.
            first_x: x coordinate of the first turbine. If None, 
                this will be assumed to be the smallest x coordinate in  the wind field. This can be used to have propagation originate from different points in the wind field, such as individual turbines.
            angle_changes: List of booleans specifying which 
                turbines have changed their yaw angles.
        """
                ...
                if not no_wake:
                    if sim_time is not None or look_ahead:
                        ...
                        if turbine.send_wake or (look_ahead and sim_time is not None):
                            self.propagate_wake_effects(turb_u_wake, turbine.number, sim_time, first_x=first_x, angle_changes=angle_changes)
                            turbine.send_wake = False

                        if look_ahead or visualize or sim_time == 0:
        ...
        if not no_wake:
            if sim_time is None or visualize:
                self.u = self.u_initial - u_wake

            if sim_time is not None and visualize:
                if not self.wind_change_resolved or sim_time == 0:
                    first_x = self.wind_map.wind_field_buffer.add_wind_field(self.u, self.mean_wind_speed, sim_time, first_x=first_x)
    ...
    @mean_wind_speed.setter
    def mean_wind_speed(self, value):
        self._mean_wind_speed = value