...
class Market():
    def __init__(self, startTimeSim, stopTimeSim, market_name, bus, data_directory=None):
        ...
        self.type_bid = 3 # start with one type
    ...
    def load_price_data(self):
        ...
        if self.market_name == 'ERCOT':
            ...
            if self.data_directory is None:
                ...
                RTpriceworkbook = pd.ExcelFile(
                    self.data_directory
                    + '/ERCOT_data/Historical/RTMLZHBSPP_' 
                    + str((startTime).year)
                    + '.xlsx'
                )
        ...
        if self.market_name == 'ERCOT':
            ...
            if self.data_directory is None:
                ...
                df_reg = pd.read_csv(
                    self.data_directory
                    + '/ERCOT_data/Historical/DAMASMCPC_'
                    + str((startTime).year)
                    + '.csv'
                )  
            df_reg = df_reg.loc[
        ...
        if self.market_name == 'ERCOT':
            ...
            if self.data_directory is None:
                ...
                priceworkbook = pd.ExcelFile(
                    self.data_directory
                    + '/ERCOT_data/Historical/DAMLZHBSPP_'
                    + str((startTime).year)
                    + '.xlsx'
                )
    ...
    def day_ahead_bidding(self, dfrt2, dfrt2AS, df_grid_inputs=None):
        ...
        DAwindforecast["quantile"] = [0.50, 0.5] # Bid the mean
        ...
        if (self.wind_input == 'FLORIS') & (self.type_bid != 1):
            ...
            for i in range(0,24): 
                for j in range(0, len(DAwindforecast)):
                # For the DA tracking test, we can just use the forecast directly
                    #DAwindforecast.at[j, i + 1]= df_grid_inputs["PWR_MEAN"].loc[
                    #            (df_grid_inputs["Delivery Hour"] == i + 1)].mean()
                    DAwindforecast.at[j, i + 1]= df_grid_inputs["quantile " + str(DAwindforecast.at[0,"quantile"])].loc[
                                (df_grid_inputs["Delivery Hour"] == i + 1)].mean()
                    # MS changed 3/13/22! TODO: CHECK W/ ES
                    #if len(
        ...
        #assumption 2 :price taker in DA market
            ...
            for hour in range(1, 25):
                dfnew2["Energy"+str(hour)] = 0
                dfnew2["Energy"+str(hour)].loc[dfnew2["RT quantile"] <= 0.5] = 0
                dfnew2["Energy"+str(hour)].loc[dfnew2["RT quantile"] > 0.5] = (
        ...
        for product in ["Energy","Regup","Regdn"]:
            if product+str(1) in dfnew2.columns: # products depend on type_bid
                for hour in range(1, 25):
                    listperhour = []
    ...
    def create_system_regulation_signal(self, create_AGC=False):
        ...
        if (self.market_name == 'SPP') & (create_AGC is True):
            ...
            # TODO: this data is not on the repo; need to get
            freq = pd.read_csv(
                #PKGROOT + "/data/ERCOT_data/ERCOT_TrueTime_BT_Hz_2s_2019.csv"
                self.data_directory + "/ERCOT_data/ERCOT_TrueTime_BT_Hz_2s_2019.csv"
            )
            ...
            if len(freqfocus) > 0:
                ...
                # TODO: this data is not on repo; need to get data
                template = pd.read_csv(
                    #PKGROOT + "/data/ERCOT_data/template_ERCOT_AGCsignal.csv"
                    self.data_directory + "/ERCOT_data/template_ERCOT_AGCsignal.csv"
                )
                ...
                template.to_csv(
                    self.data_directory + "/ERCOT_data/Synthetic_data/" + self.market_name
                    #PKGROOT + "/data/ERCOT_data/Synthetic_data/" + self.market_name
            ...
            # )
            AGC = pd.read_csv(
                self.data_directory + '/'+ self.market_name + "_data/Synthetic_data/"
                + self.market_name + "_syntheticAGCsignal_"
                + startTime.replace("-", "_") + ".csv"
            ) # MS 11/10/21
