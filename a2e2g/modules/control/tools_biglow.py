...
def get_layout(filename):
    ...
    if plot:
        ...
        for i in range(len(df_layout)):
            if 'tower' in df_layout['Tag'].iloc[i]:
                plt.plot(df_layout['easting'].iloc[i] - np.min(np.array(df_layout['easting'])), \
                         df_layout['northing'].iloc[i] - np.min(np.array(df_layout['northing'])), 'ko')
                ...
                plt.plot(df_layout['easting'].iloc[i] - np.min(np.array(df_layout['easting'])), \
                         df_layout['northing'].iloc[i] - np.min(np.array(df_layout['northing'])), 'ro')
